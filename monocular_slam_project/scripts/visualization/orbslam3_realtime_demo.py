#!/usr/bin/env python3
"""Pseudo-real-time ORB-SLAM3 demo using saved frames and TUM trajectory output."""
...
def load_tum_trajectory(trajectory_path):
    if not trajectory_path.exists():
        raise FileNotFoundError(f"Trajectory file does not exist: {trajectory_path}")
    if not trajectory_path.is_file():
        raise ValueError(f"Trajectory path is not a file: {trajectory_path}")
    ...
    if not poses:
        raise ValueError(f"Trajectory file has no valid poses: {trajectory_path}")
    return poses
...
def add_status_overlay(frame, frame_index, total_frames, visible_poses, total_poses, fps, sync_mode):
    import cv2

    overlay = frame.copy()
    lines = [
        "ORB-SLAM3 monocular demo (headless replay)",
        f"Frame: {frame_index + 1}/{total_frames}",
        f"Trajectory keyframes shown: {visible_poses}/{total_poses}",
        f"Replay FPS: {fps:g} | Sync: {sync_mode} | Press q or Esc to quit",
    ]
...
def trajectory_axes_limits(poses):
    tx = [pose[1] for pose in poses]
    tz = [pose[3] for pose in poses]
    x_min, x_max = min(tx), max(tx)
    z_min, z_max = min(tz), max(tz)
    margin = max(x_max - x_min, z_max - z_min, 1e-3) * 0.15
    return (x_min - margin, x_max + margin), (z_min - margin, z_max + margin)


def render_trajectory_plot(poses, visible_count, size, axes_limits):
    import cv2
    import numpy as np
    ...
    axis.set_title("ORB-SLAM3 trajectory")
    axis.set_xlabel("x translation")
    axis.set_ylabel("z translation")

    all_x = [pose[1] for pose in poses]
    all_z = [pose[3] for pose in poses]
    axis.plot(all_x, all_z, color="0.82", linewidth=1.0, label="full saved path")

    if visible_count > 0:
        visible = poses[:visible_count]
        x_values = [pose[1] for pose in visible]
        z_values = [pose[3] for pose in visible]
        axis.plot(x_values, z_values, color="#1f77b4", linewidth=2.2, label="revealed path")
    ...
    plot_rgba = cv2.cvtColor(rgba, cv2.COLOR_RGBA2BGR)
    return plot_rgba
...
def visible_pose_count_timestamp(poses, frame_index, fps):
    frame_time = frame_index / fps
    count = 0
    for pose in poses:
        if pose[0] <= frame_time:
            count += 1
        else:
            break
    return count
...
def visible_pose_count_even(poses, frame_index, total_frames):
    progress = min(max((frame_index + 1) / total_frames, 0.0), 1.0)
    return min(len(poses), max(1, int(round(progress * len(poses)))))


def visible_pose_count(poses, frame_index, total_frames, fps, sync_mode):
    if sync_mode == "even":
        return visible_pose_count_even(poses, frame_index, total_frames)
    return visible_pose_count_timestamp(poses, frame_index, fps)


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Replay extracted frames with an updating ORB-SLAM3 trajectory plot. "
            "This is a pseudo-real-time visualization using saved results."
        )
    )
    parser.add_argument(
        "--frames",
    ...
    parser.add_argument(
        ...
        help="ORB-SLAM3 TUM trajectory file. Default: indoor_loop keyframe trajectory.",
    )
    ...
    parser.add_argument(
        "--sync-mode",
        choices=["timestamp", "even"],
        default="timestamp",
        help=(
            "How to reveal saved keyframes. 'timestamp' follows ORB-SLAM3 timestamps "
            "and may pause during tracking gaps. 'even' spreads keyframes across the "
            "video for smoother presentation playback. Default: timestamp."
        ),
    )
...
def main():
    ...
    axes_limits = trajectory_axes_limits(poses)
    window_name = "ORB-SLAM3 pseudo-real-time demo"
    ...
    print(f"Trajectory keyframes: {len(poses)}")
    ...
    for frame_index in range(0, len(frames), args.stride):
        ...
        visible_count = visible_pose_count(
            poses,
            frame_index,
            len(frames),
            args.fps,
