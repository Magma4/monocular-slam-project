...
def to_tensor(ndarray, volatile=False, requires_grad=False, dtype=torch.float32, device=DEVICE):
    return Variable(
...
def train_model(model, train_loader, criterion, optimizer, epoch, batch_size, trajectory_length):
    ...
    for batch_idx, (images_stacked, odometries_stacked) in enumerate(train_loader):
        images_stacked = images_stacked.to(DEVICE, dtype=torch.float32)
        odometries_stacked = odometries_stacked.to(DEVICE, dtype=torch.float32)
        ...
        estimated_odometries = Variable(torch.zeros(odometries_stacked.shape, device=DEVICE))
...
def train(model, datapath, checkpoint_path, epochs, trajectory_length, args):
    ...
    train_loader = torch.utils.data.DataLoader(VisualOdometryDataLoader(datapath, trajectory_length=trajectory_length, transform=preprocess, image_folder=args.image_folder), batch_size=args.bsize, shuffle=True, drop_last=True, **kwargs)
...
def test(model, datapath, trajectory_length, validation_steps, preprocess, image_folder):
    model.eval()
    model.training = False

    kwargs = {'num_workers': 1}
    test_loader = torch.utils.data.DataLoader(VisualOdometryDataLoader(datapath, trajectory_length=trajectory_length, transform=preprocess, test=True, image_folder=image_folder), batch_size=1, shuffle=True, **kwargs)
    ...
    with open(os.path.join(datapath, "index.txt"), 'r') as reader:
        for index in reader:
            ...
            for image_index in range(len(images_path)-1):
                ...
                images_stacked = Variable(images_stacked).to(DEVICE)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_folder', default='image_0', type=str, help='KITTI camera folder to read, e.g. image_0, image_1, or image_2')
    ...
    if args.mode == 'train':
        ...
        test(model, args.datapath, args.trajectory_length, args.validation_steps, preprocess, args.image_folder)
    else:
