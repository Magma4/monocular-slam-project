...
class DeepVONet(nn.Module):
    ...
    def reset_hidden_states(self, size=1, zero=True):
        ...
        if zero == True:
            self.hx1 = Variable(torch.zeros(size, 100, device=device))
            self.cx1 = Variable(torch.zeros(size, 100, device=device))
            self.hx2 = Variable(torch.zeros(size, 100, device=device))
            self.cx2 = Variable(torch.zeros(size, 100, device=device))
        else:
            self.hx1 = Variable(self.hx1.data.to(device))
            self.cx1 = Variable(self.cx1.data.to(device))
            self.hx2 = Variable(self.hx2.data.to(device))
            self.cx2 = Variable(self.cx2.data.to(device))
